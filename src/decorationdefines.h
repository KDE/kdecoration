 ...
 *
 * \value Menu The Menu button requests showing the window menu on left or right click.
 * \value ApplicationMenu The ApplicationMenu button requests showing the application's menu on left or right click.
 * \value OnAllDesktops The OnAllDesktops button requests toggling the DecoratedClient's on all desktops state. The DecoratedButton is only visible if
 *multiple virtual desktops are available.
 * \value Minimize The Minimize button requests minimizing the DecoratedClient. The DecorationButton is only
 * enabled if the DecoratedClient is minimizeable.
 * \value Maximize The Maximize button requests maximizing the DecoratedClient. The DecorationButton is checkable
 ...
 * \value Close The Close button requests closing the DecoratedClient. The DecorationButton is only enabled
 * if the DecoratedClient is closeable.
 * \value ContextHelp The ContextHelp button requests entering the context help mode. The DecorationButton is only
 * visible if the DecoratedClient provides context help.
 * \value Shade The Shade button requests toggling the DecoratedClient's shaded state. The DecoratedButton
 * is only enabled if the DecoratedClient is shadeable.
 * \value KeepBelow The KeepBelow button requests toggling the DecoratedClient's keep below state.
 * \value KeepAbove The KeepAbove button requests toggling the DecoratedClient's keep above state.
 * \value Custom The Custom type allows a Decoration to provide custom DecorationButtons.
 * \value Spacer The Spacer button provides some space between buttons.
 **/
